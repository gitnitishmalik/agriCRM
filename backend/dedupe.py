...
Moved from `apps/organisations/dedupe.py` during the FastAPI migration. The
scorer itself — normalisation, token sets, the Dice coefficient — is pure
Python and is unchanged, deliberately: it encodes real decisions about Indian
name matching (transliteration variants, honorific stripping, legal-form
suffixes) and rewriting it would have risked all of them for nothing.

Only the candidate query changed. Django expressed the trigram prefilter as
`TrigramSimilarity(...)`; here it is the `similarity()` function the
`pg_trgm` extension provides, written as SQL. Same index, same threshold,
same meaning.
"""
...
async def find_duplicates(
    ...
    *,
    ...
    statement = select(Organisation).where(Organisation.is_deleted.is_(False))

    district_scoped = district_id is not None
    if district_scoped:
        statement = statement.where(Organisation.district_id == district_id)
    elif state_id is not None:
        statement = statement.where(Organisation.state_id == state_id)
    if exclude_id is not None:
        statement = statement.where(Organisation.id != exclude_id)
    ...
    if district_scoped:
        ...
        statement = statement.order_by(Organisation.name).limit(CANDIDATE_LIMIT * 40)
    else:
        # `similarity()` from pg_trgm, index-backed. The Django ORM spelled
        # this `TrigramSimilarity`; it is the same function underneath.
        score = func.similarity(Organisation.name, name)
        statement = (
            statement.where(score >= PREFILTER_THRESHOLD)
            .order_by(score.desc())
